...
@st.cache_data 
def load_data(): 
    try: df = pd.read_csv("db_growth_data.csv", parse_dates=["Date"]) 
        return df 
...
for _, row in server_sizes.iterrows(): 
    ...
    st.write(f"### Total Across All Servers: **{total_size:.2f} GB**") st.markdown("---") 
# Inputs 
...
if selected_server != "All Servers": 
    ...
    db_list = ["All Databases"] selected_database = st.selectbox("Select Database", db_list) 
...
def forecast_arima(ts, periods): 
    ...
    return pd.DataFrame({ "Date": future_dates, "Forecast_GB": forecast_mean.values, "Lower_Bound": conf_int.iloc[:, 0].values, "Upper_Bound": conf_int.iloc[:, 1].values }) 
# Trigger Forecast 
...
ax.fill_between(forecast_df["Date"], forecast_df["Lower_Bound"], forecast_df["Upper_Bound"], color='red', alpha=0.2, label="Confidence Interval") 
# Capacity limit line 
